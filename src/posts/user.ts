...
interface PostsObject {
    ...
    overrideGuestHandle: (postData: PostData, handle: Handle) => void;
    ...
    isModerator: (pids: number[] | number, uid: string) => Promise<boolean[] | boolean>;
    changeOwner: (pids: number[] | number, toUid: string|number) => Promise<PostData[]>;
...
export = function (Posts: PostsObject) {
    ...
    async function parseSignature(userData: UserData, uid: number, signatureUids): Promise<string> {
    ...
    Posts.isOwner = async function (pids, uid) {
        ...
        const postData = await Posts.getPostsFields(pids, ['uid']) as PostData[];
    ...
    async function reduceCounters(postsByUser:PostData[][]): Promise<void> {
        // The next line calls a function in a module that has not been updated to TS yet
        // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call
    ...
    async function updateTopicPosters(postData: PostData[], toUid: number): Promise<void> {
